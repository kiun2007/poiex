...
public class VarAstNode extends AbstFelNode  {
	...
	public Object invoke(Object obj, Class<?> contextClass, FelContext context) {
		...
		if (obj instanceof Map){
			return ((Map<?, ?>) obj).get(getText());
		}

		int hash = Objects.hash(contextClass);
		...
		if (method == null){
			...
			method = ReflectUtil.findMethod(contextClass, methodName, null);
			lastHash = hash;
			methodCache = method;
		...
		if (method != null){
			return Dot.invoke(obj, method, new Object[]{});
		}
		return null;
	}
}
